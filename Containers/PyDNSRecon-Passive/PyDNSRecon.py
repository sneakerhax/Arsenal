...
def amass_dns_passive(amass_target, amass_output, domain_list):
    print("[+] Running Amass passive scan on " + amass_target)
    amass = subprocess.Popen([amass_binary, 'enum', '-passive', '-d', amass_target, '-o', amass_output], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    amass.communicate()
...
def run(target):
    print("[*] Passive scans may contain stale DNS records!")
    try:
        ...
        pass
    try:
        amass_dns_passive(target, amass_output, domain_list)
    except Exception as e:
